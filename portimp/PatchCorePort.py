...
class PatchCorePort(AlgorithmPort):
    ...
    #         # 第一版
    #         # 需要添加一个flag决定是否使用preprocess
    #         train_img_paths = self._collect_images_from_folders(ok_image_folders)
    #         if not train_img_paths:
    #             raise ValueError(f"No images found in ok_image_folders: {ok_image_folders}")

    #         # ===== 创建 dataset 之前：先对所有对齐图做 imagepro_for_AIdetect（不 resize）=====
    #         # 选一个 cache 目录：建议放到本次训练输出目录下，方便复现实验
    #         base_out = output_root or getattr(self.cfg, "output_root", "./_export_models")
    #         pre_cache_dir = os.path.join(base_out, f"_pre_cache_{model_id}")

    #         # 如果你的 ok_image_folders 只有一个根目录，可以把 keep_rel_to 设成它来保留相对路径
    #         keep_rel = ok_image_folders[0] if len(ok_image_folders) == 1 else None

    #         pre_img_paths = preprocess_to_cache_no_resize(
    #             train_img_paths,
    ...
    #         # ---- dataloader ----
    #         # dataset = NormalImageDataset(train_img_paths, self.transform)
    #         dataset = NormalImageDataset(pre_img_paths, self.transform)
    #         loader = DataLoader(
    #             dataset,
    ...
    #             features = apply_random_projection(features, projection_matrix)  # 不再传 should_stop
    #         # ---- stage3: coreset ----
    ...
    #             "train_images": int(len(train_img_paths)),
    #             "train_image_count": int(len(train_img_paths)),  # 兼容你上层 UI 习惯
...
# def preprocess_to_cache_no_resize(
#     img_paths: List[str],
#     *,
#     cache_dir: str,
#     blur: bool = False,
#     contrast: float = 1.2,
#     roi: Optional[Tuple[int, int, int, int]] = None,  # (x1,y1,x2,y2) 对齐后坐标
#     keep_rel_to: Optional[str] = None,                # 用于保留相对目录结构，避免同名覆盖
# ) -> List[str]:
#     """
#     对齐后的图 -> imagepro_for_AIdetect（不 resize）-> 转 3 通道 -> 保存到 cache_dir
#     返回预处理后的图片路径列表，用于创建 dataset。
#     """
...
#         # 灰度 -> 3通道（给 torchvision/resnet 用）
#         out_bgr = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)

#         # 生成输出路径：尽量保留相对目录结构，避免同名覆盖
#         if keep_rel_to:
#             try:
